...
import { Box, Button, Input, ListItem } from '@mui/material';
    ...
    const handleReturnSubmit = () => {
      ...
      apiClient.updateLoan(updatedLoanData).then((response: any) => {
        ...
        if (response.success) {
          ...
          updatedLoans[selectedLoanIndex].wasReturned = modalAction === 'return';
          updatedLoans[selectedLoanIndex].returnDate = updatedLoanData.returnDate;  // Update the returnDate in the local state
        ...
        <div className="content">
          ...
          >
            {loans.map((loan, index) => (
              <ListItem key={index} style={{ width: '100%', maxWidth: 650 }}>
                <ListItemAvatar>
                  <Avatar
                    style={{
                      background: checkIfReturned(loan.returnDate, loan.wasReturned),
                    }}
                  >
                    <BookIcon />
                  </Avatar>
                ...
                <ListItemText
                  ...
                  secondary={`${t('rented_by')} ${
                    loan.user.id
                  }, ${t('from')} ${loan.startDate} ${t('to')} ${loan.endDate}`}
                />
                <Button
                  variant="contained"
                  color={loan.wasReturned ? 'secondary' : 'primary'}
                  onClick={() =>
                    handleReturnClick(index, loan.wasReturned ? 'undo' : 'return')
                  }
                >
                  {loan.wasReturned ? t('undo_return') : t('mark_as_returned')}
                </Button>
              </ListItem>
            ))}
          </List>
        ...
        <Modal open={isModalOpen} onClose={handleModalClose}>
          ...
          >
            ...
            <div className="modal-button">
              <Button onClick={handleReturnSubmit} variant="outlined" size="large">
